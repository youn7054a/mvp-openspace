...
- **주제 목록** — 핀으로 꽂힌 **스티커 카드 벽**, 배정된 주제를 날짜·시간·테이블 순으로 먼저 표시하고 배정 정보·상태 칩 노출
- **타임슬롯 일괄 생성** — 날짜·시작시각·길이·휴식·개수로 연속 슬롯 한 번에 (실시간 미리보기)
- **타임테이블 짜기** (`/admin/timetable`) — 워드 표처럼 ＋로 시간(행)은 아래로·방(열)은 옆으로 추가, 칸을 눌러 즉석 수정, ✕로 삭제하는 그리드 빌더
- **슬롯 닫기/라벨** — "키노트", "휴식" 등 시간 전체 또는 **특정 룸 셀만** 커스텀 라벨로 닫아 예약 불가 처리
- **이벤트와 대화** — 대화 주제는 테이블+시간에, 이벤트는 시간에만 배정하여 같은 시간대 표에 함께 표시
- **관리자 직접 배정** — 관리자가 주제를 슬롯에 배정/해제 (더블부킹 차단), 주제 목록에서 신청자 이메일 확인
...
- **라이트닝토크** (`/light`) — 날짜별 현장 신청, 관리자 합격·발표 순서·자료 URL 관리, 개인정보 없는 안내 전광판
...
| `/topics` | 주제 목록 (Topic List) — 스티커 벽, 배정된 주제 우선 정렬 (공개) |
...
| `/admin/board` | 전광판 QR — QR 코드 추가·수정·삭제 |
| `/admin/auto-board` | 자동 전광판 URL 관리 — URL·표시 시간·순서 등록/삭제 |
| `/admin/light` | 라이트닝토크 관리 — 날짜별 접수, 신청자·이메일, 합격/불합격·발표 순서·자료 URL·QR 관리 |
| `/logout` (POST) | 로그인 세션 해제 (`/auth/check` = 로그인 완료 확인 프로브) |
...
5. 관리자(`/admin`)는 타임테이블 짜기(그리드 빌더)·룸/타임슬롯 생성, 슬롯 닫기/라벨, 주제 숨김/삭제, 배정, 신청자 이메일 확인, 전광판 QR·자동 전광판 URL·라이트닝토크 접수를 관리

스케줄 무결성은 DB 유니크 제약으로 보장됩니다
(`unique(topic_id)`, `unique(room_id, timeslot_id)`) — 더블부킹/중복 등록 차단.

보안: 소유권은 **PyCon 회원 id(서버 검증)** 로 확인하고, `host_email`은 공개 화면에는 노출하지 않으며 관리자 `/admin/topics`에서만 확인할 수 있고,
